...
const isOwnerOrAdmin = (user: IUser | undefined, bookAuthorId: unknown): boolean => {
...
export const createChapter = async (bookId: string, user: IUser | undefined, payload: ChapterPayload) => {
    const book = await getBookOrThrow(bookId);
    if (!isOwnerOrAdmin(user, book.authorId)) throw new AppError("UnAuthorized", 401);
...
export const listChapters = async (bookId: string, user: IUser | undefined, status?: "draft" | "published") => {
    const book = await getBookOrThrow(bookId);
    ...
    const orderMap = new Map(book.chapters.map((chapterId, index) => [chapterId.toString(), index]));
...
export const getChapterById = async (bookId: string, chapterId: string, user: IUser | undefined) => {
    const book = await getBookOrThrow(bookId);
    const ownerOrAdmin = isOwnerOrAdmin(user, book.authorId);
...
export const deleteChapter = async (bookId: string, chapterId: string, user: IUser | undefined) => {
    const book = await getBookOrThrow(bookId);
    if (!isOwnerOrAdmin(user, book.authorId)) throw new AppError("UnAuthorized", 401);
...
export const findChaptersOfBook = async (bookId: string, authorId: string) => {
    const book = await getBookOrThrow(bookId);
    if (book.authorId.toString() !== authorId) throw new AppError("Unauthorized", 401);
...
export const reorderChapters = async (bookId: string, user: IUser | undefined, newOrderIds: string[]) => {
    const book = await getBookOrThrow(bookId);
    if (!isOwnerOrAdmin(user, book.authorId)) throw new AppError("UnAuthorized", 401);
    ...
    const isSameSet =
        new Set(oldOrderIds).size === new Set(newOrderIds).size && newOrderIds.every((id) => oldOrderIds.includes(id));

    if (!isSameSet) throw new AppError("Invalid ordering", 400);

    await book.updateOne({ $set: { chapters: newOrderIds } });

    return ChapterModel.find({ bookId: book._id }).select("_id title wordCount status").lean();
};
